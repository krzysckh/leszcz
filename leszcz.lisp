...
(defun game-propose-draw (g)
  (declare (type game g))
  (when-let ((c (game-connection g)))
    (write-packets c (make-client-packet 'gdata :gdata-drawp t))))

(defun game-accept-draw (g)
  (when-let ((c (game-connection g)))
    (write-packets c (make-client-packet 'gdata :gdata-draw-ok t))
    (setf (game-result g) 'draw)))
...
(defun draw-game-control-buttons (g &rest _)
  ...
  (let ((rec (list (car dmh/rect) (+ (cadr dmh/rect) (cadddr dmh/rect) dt/font-size) (caddr dmh/rect) 32)))
    ...
    (when *opponent-proposed-draw-p*
      (when (draw-icon 'draw-p (+ (car rec) 32 8 32 8 32 8 32 8) (cadr rec) 32 32 "zaakceptuj remis") ; lepiej remis niz w dupe penis
        (game-accept-draw g)
        (display-draw g "po dogadaniu")))
    ))
  ...
  (when-let ((c (game-connection game)))
    (when-let ((p (maybe-receive-packet (game-connection game))))
      ...
      (packet-case p
        ...
        (gdata
         (cond
           ((fast:bit-set-p (aref p 0) 7 :type-size 8)
           ...
           ((fast:bit-set-p (aref p 1) 2 :type-size 8)
            (error "Received takeback-ok-ok when literally anything else expected."))
           ((fast:bit-set-p (aref p 1) 3 :type-size 8)
